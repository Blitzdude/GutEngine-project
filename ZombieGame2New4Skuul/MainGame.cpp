...
void MainGame::initLevel() {
    ...
    for (int i = 0; i < m_levels[m_currentLevel]->getNumHumans(); i++) {
        ...
        glm::vec2 pos(randX(m_randomEngine) * TILE_WIDTH, randY(m_randomEngine) * TILE_WIDTH);
    ...
    for (int i = 0; i < zombiePositions.size(); i++) {
        ...
        m_zombies.back()->init(ZOMBIE_SPEED, zombiePositions[i], 2.0f);
    }
...
void MainGame::updateAgents(float deltaTime) {
	...
	std::uniform_real_distribution<float> randS(1.0f, 2.5f);
    ...
    for (int i = 0; i < m_zombies.size(); i++) {
        ...
        for (int j = 1; j < m_humans.size(); j++) {
            if (m_zombies[i]->collideWithAgent(m_humans[j])) {
                // Add the new zombie
                m_zombies.push_back(new Zombie);
				m_zombies.back()->init(ZOMBIE_SPEED, m_humans[j]->getPosition(), randS(m_randomEngine));
                // Delete the human
                delete m_humans[j];
